...
- (CGFloat)heightWithText:(NSString *)stringText font:(CGFloat)font {
    NSString *whiteString = [stringText stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    CGSize constraint = CGSizeMake(self.contentSize.width , CGFLOAT_MAX);
    [self sizeToFit];
    CGRect twoSize = [whiteString boundingRectWithSize:constraint options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading) attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:font]} context:nil];
    CGFloat textHeight = twoSize.size.height +22 ;
    return textHeight;
}
@end
